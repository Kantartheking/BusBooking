...
def show_bus():
    if to.get() == "" and fro.get() == "" and journey_date.get() == "":
        ...
        cur = con.cursor()
        # cur.execute('''Select Name,Bus_type,Seat_available,Capacity,Fare from Bus_details as b,Operator_details as o,Running_details,
        #       Route_details as f, Route_details as t where f.Sname=? and t.Sname=?
        #       and f.S_Id<t.S_Id and f.R_Id=b.Op_Id and o.Op_Id=b.Op_Id ''', (fro.get(), to.get()))
    ...
    def proceed():
        ...
        # transaction_id = random.randint(100, 200)

        # cur.execute("insert into Booking_history values('"+name.get()+"','"+gender_type.get() +
        #             "','"+no_of_seats .get()+"','"+mobile_number.get() + "','"+age.get()+"','"+b_id.get()+"')")

        m = int(mobile_number.get())
        n = name.get()
        ns = int(no_of_seats.get())
        g = gender_type.get()
        a = age.get()
        b = b_id.get()
        d = date.today()
        ...
        print(type(f))
        print(type(o))
        # cur.execute('''insert into Booking_history(Phone,Passenger_name ,No_of_passenger,Gender,Age)values(?,?,?,?,?)''', (m, n, ns, g, a))
        cur.execute('''insert into Booking_history(Phone,Passenger_name ,No_of_passenger,Gender,Age,B_Id,Date,Journey_date,Boarding_station,Booking_ref)values(?,?,?,?,?,?,?,?,?,?)''', (m, n, ns, g, a, b, d, j, f, o))
        # print(bus_ticket.get())
# # # cur.execute("INSERT INTO Operator_details(Op_Id,Name,Address,Email,Phone)VALUES(1,)")))

        # cur.execute('''insert into Booking_history(Phone,Passenger_name ,No_of_passenger,Gender,Age,B_Id)
        #     values(?,?,?,?,?,?);''',[mobile_number.get(), name, no_of_seats,gender_type, age))

        con.commit()

        cur.execute("update Running_details set Seat_available=Seat_available-No_of_passenger from Running_details as r ,Booking_history as b where b.B_Id=r.B_Id and b.Journey_date=r.Date")
        con.commit()
        con.close()
        # ticket = cur.fetchall()
        # print("Success")
        # else:
        #     messagebox.showinfo("", "Please fill all the field")
